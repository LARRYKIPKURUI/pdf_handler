...
def merge_pdfs(input__folder, output_pdf):
    # remove existing merge.pdf if existing
    if os.path.exists(output_pdf):
        os.remove(output_pdf)

    # Get a list of pdfs in the input dir
    pdf_files = [file for file in os.listdir(input__folder) if file.endswith(".pdf")]
    ...
    for pdf in pdf_files:
        with open(os.path.join(input__folder, pdf), "rb") as file:
